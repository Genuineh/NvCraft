...
local function topological_sort(dependencies)
	...
	local function visit(module_name)
		if not module_name or not dependencies[module_name] then
			-- This can happen if a dependency is unknown/mistyped.
			return true
		end
		...
		if not visited[module_name] then
			...
			for _, dep in ipairs(dependencies[module_name]) do
...
function M.discover_modules()
	...
	-- First pass: Load all specs and build a map from short names to full names
	...
	for _, module_name in ipairs(discovered_modules) do
		...
		if spec then
			...
			local short_name = module_name:match("([^.]+)$")
			if short_to_full_map[short_name] then
				vim.notify(
					"Warning: Duplicate module short name '"
						.. short_name
						.. "'. Resolution may be ambiguous.",
					vim.log.levels.WARN
				)
			end
			short_to_full_map[short_name] = module_name
		end
	end

	-- Second pass: Build dependency graph using the map to resolve full names
	local dependencies = {}
	for module_name, spec in pairs(module_specs) do
		local full_deps = {}
		if spec.dependencies then
			for _, dep_name in ipairs(spec.dependencies) do
				-- A dependency can be a short name or a full name
				local full_dep_name = short_to_full_map[dep_name] or dep_name
				if module_specs[full_dep_name] then
					table.insert(full_deps, full_dep_name)
				else
					vim.notify(
						"Module '" .. module_name .. "' has an unresolved dependency: '" .. dep_name .. "'",
						vim.log.levels.WARN
					)
				end
			end
		end
		dependencies[module_name] = full_deps
	end
...
function M.check_version_compatibility(dependent_spec, dependency_spec)
	local required_version = dependent_spec.dependency_versions
		and dependent_spec.dependency_versions[dependency_spec.name]
